...
AWSConfig.region = AUTH.region

// Creates a CognitoAuth instance
const createCognitoAuth = () => {
  const appWebDomain = AUTH.userPoolBaseUri.replace('https://', '').replace('http://', '')
  const auth = new CognitoAuth({
    UserPoolId: AUTH.userPool,
    ClientId: AUTH.clientId,
    AppWebDomain: appWebDomain,
    TokenScopesArray: AUTH.tokenScopes,
    RedirectUriSignIn: AUTH.callbackUri,
    RedirectUriSignOut: AUTH.signoutUri
  })
  return auth
}
...
const getCognitoSignInUri = () => {
  const signinUri = `${AUTH.userPoolBaseUri}/login?response_type=code&client_id=${AUTH.clientId}&redirect_uri=${AUTH.callbackUri}`
  return signinUri
}
